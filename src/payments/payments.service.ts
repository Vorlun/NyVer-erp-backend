...
export class PaymentsService {
  ...
  async findAll(query: QueryPaymentDto) {
    ...
    if (query.dateFrom || query.dateTo) {
      where.paidForDate = {};
      if (query.dateFrom) where.paidForDate.gte = new Date(query.dateFrom);
      if (query.dateTo) where.paidForDate.lte = new Date(query.dateTo);
    }
  ...
  async processOnlineCheckout(dto: OnlineCheckoutDto) {
    ...
    // Handle receipt screenshot if provided as base64
  ...
  saveReceiptBuffer(file: { buffer: Buffer; originalname: string }): string {
  ...
  async approvePayment(id: number) {
    ...
    // If student didn't have a temp password, generate one so admin/student can use it
    if (!tempPassword) {
      tempPassword = 'NyVer#' + Math.floor(1000 + Math.random() * 9000);
      const hashedPassword = await bcrypt.hash(tempPassword, 10);
    ...
    // Auto enroll in matching course group if courseId exists
    let group = null;
    if (payment.courseId) {
  ...
  /**
   * Admin tomonidan foydalanuvchi balansiga pul qo'shish
   * POST /users/:id/balance/top-up
   */
  ...
  /**
   * Foydalanuvchi o'z balansi orqali kursni to'liq sotib oladi
   * POST /payments/pay-from-balance
   */
  async payFromBalance(dto: PayFromBalanceDto) {
    const student = await this.prisma.user.findUnique({
      where: { id: dto.studentId },
    });
    ...
    if (balance < price) {
      throw new BadRequestException(
        `Balansda yetarli mablag' yo'q. Kerak: ${price.toLocaleString()} so'm, Mavjud: ${balance.toLocaleString()} so'm`,
      );
    }

    // Atomik tranzaksiya: balansdan yechib payment + enrollment yaratish
    const [payment, enrollment] = await this.prisma.$transaction(async (tx) => {
      // 1. Balansdan yechib olish
      await tx.user.update({
        where: { id: dto.studentId },
        data: { balance: { decrement: price } },
      });

      // 2. To'lov yozuvi yaratish
      ...
      // 3. Guruhga biriktirish (agar aktiv guruh bo'lsa)
      let enr = null;
