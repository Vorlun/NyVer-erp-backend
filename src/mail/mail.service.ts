...
export class MailService {
  ...
  constructor(private configService: ConfigService) {
    ...
    const secure = port === 465 || this.configService.get<string>('MAIL_SECURE') === 'true';
    const user = this.configService.get<string>('MAIL_USER');
    const pass = this.configService.get<string>('MAIL_PASS');
    const envFrom = this.configService.get<string>('MAIL_FROM');

    // Gmail SMTP talab qiladi: "from" manzili aynan autentifikatsiyadan o'tgan Gmail bilan bir xil bo'lishi kerak.
    // Aks holda SPF/DKIM xatosi sababli xat Spamga tushadi yoki qabul qilinmaydi.
    ...
    if (host && user && pass) {
      ...
      this.logger.warn(
        `SMTP ma'lumotlari to'liq kiritilmagan (MAIL_HOST, MAIL_USER, MAIL_PASS). Xatlar server konsoliga Preview rejimida chiqariladi.`,
      );
    }
  }

  /**
   * Yangi ro'yxatdan o'tgan talabaga kirish ma'lumotlari (Login & Vaqtinchalik parol) bilan xat yuborish
   */
  async sendWelcomeCredentialsEmail(options: WelcomeCredentialsOptions): Promise<boolean> {
...
<body>
  ...
  /**
   * Avval ro'yxatdan o'tgan talabaga yangi kursga qo'shilganlik haqida xat yuborish
   */
  async sendEnrollmentConfirmationEmail(options: EnrollmentConfirmationOptions): Promise<boolean> {
...
© NyVer O'quv Markazi`;
  ...
  private async dispatchMail(params: {
    to: string;
    subject: string;
    html: string;
    text?: string;
    ...
    // Log to console in formatted card for full visibility
    this.logger.log(`
===================== [NYVER MAIL DISPATCH] =====================
Turi: ${debugInfo.type}
Kimga (To): ${to}
...
=================================================================
    ...
    if (!this.transporter) {
      this.logger.log(
        `[MOCK PREVIEW]: SMTP ulanmaganligi sababli xat real jo'natilmadi, lekin yuqoridagi konsolda to'liq qayd etildi.`,
      );
      return true;
    }
    ...
    try {
      const res = await this.transporter.sendMail({
        from: this.mailFrom,
        replyTo: this.configService.get<string>('MAIL_REPLY_TO') || 'noreply@nyver.uz',
        to,
        subject,
